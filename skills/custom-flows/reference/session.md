...
As soon as a [`User`](https://clerk.com/docs/pr/core-3/reference/javascript/user) signs in, Clerk creates a `Session` for the current [`Client`](https://clerk.com/docs/pr/core-3/reference/javascript/client). Clients can have more than one sessions at any point in time, but only one of those sessions will be **active**.

In certain scenarios, a session might be replaced by another one. This is often the case with [multi-session applications](https://clerk.com/docs/pr/core-3/guides/secure/session-options#multi-session-applications).

All sessions that are **expired**, **removed**, **replaced**, **ended** or **abandoned** are not considered valid.

> \[!NOTE]
> For more information regarding the different session states, see the [guide on session management](https://clerk.com/docs/pr/core-3/guides/secure/session-options).
...
`user` [`User`](https://clerk.com/docs/pr/core-3/reference/javascript/user)

The user associated with the session.

***

`publicUserData` [`PublicUserData`](https://clerk.com/docs/pr/core-3/reference/javascript/types/public-user-data)

Public information about the user that this session belongs to.

***

`status` [`SessionStatus`](https://clerk.com/docs/pr/core-3/reference/javascript/types/session-status)
...
The time the session was last active on the [`Client`](https://clerk.com/docs/pr/core-3/reference/javascript/client).
...
The JWT actor for the session. Holds identifier for the user that is impersonating the current user. Read more about [impersonation](https://clerk.com/docs/pr/core-3/guides/users/impersonation).

***

`currentTask` <code><a href="https://clerk.com/docs/pr/core-3/reference/javascript/types/session-task">SessionTask</a> | null</code>

The current pending task for the session. Read more about [session tasks](https://clerk.com/docs/pr/core-3/guides/configure/session-tasks).
...
Attempts to complete the first factor verification process. Returns a [`SessionVerification`](https://clerk.com/docs/pr/core-3/reference/javascript/types/session-verification) instance with its status and supported factors.
...
Attempts to complete the second factor verification process. Returns a [`SessionVerification`](https://clerk.com/docs/pr/core-3/reference/javascript/types/session-verification) instance with its status and supported factors.
...
Accepts [Role](https://clerk.com/docs/pr/core-3/guides/organizations/control-access/roles-and-permissions#roles) key.

***

`permission` `string`

Accepts [Permission](https://clerk.com/docs/pr/core-3/guides/organizations/control-access/roles-and-permissions#permissions) key.

***

`feature` `string`

Accepts [Feature](https://clerk.com/docs/pr/core-3/guides/secure/features) key.

***

`plan` `string`

Accepts [Plan](https://clerk.com/docs/pr/core-3/guides/billing/overview) key.

***

`reverification?` <code>ReverificationConfig</code>
...
For more information, see the [guide on authorization checks](https://clerk.com/docs/pr/core-3/guides/secure/authorization-checks).
...
For more information, see the [guide on reverification](https://clerk.com/docs/pr/core-3/guides/secure/reverification).
...
Retrieves the current user's [session token](https://clerk.com/docs/pr/core-3/guides/sessions/session-tokens) or a [custom JWT template](https://clerk.com/docs/pr/core-3/guides/sessions/jwt-templates).
...
Initiates the first factor verification process. This is a required step to complete a reverification flow when using a preparable factor. Returns a [`SessionVerification`](https://clerk.com/docs/pr/core-3/reference/javascript/types/session-verification) instance with its status and supported factors.
...
Initiates the second factor verification process. This is a required step to complete a reverification flow when using a preparable factor. Returns a [`SessionVerification`](https://clerk.com/docs/pr/core-3/reference/javascript/types/session-verification) instance with its status and supported factors.
...
Initiates the reverification flow. Returns a [`SessionVerification`](https://clerk.com/docs/pr/core-3/reference/javascript/types/session-verification) instance with its status and supported factors.
